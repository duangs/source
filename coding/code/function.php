...
function getRelativePath($a, $b){
	...
	$afile = array_pop($aarr);
	$bfile = array_pop($barr);
	...
	return $pathinfo.$path.$bfile;
}

/**
 * 约瑟夫环
 * @param  [type] $num   人数
 * @param  [type] $timer 数到第几位踢出
 * @return [type]        [description]
 */
function fuhuan($num, $timer) {
	function run($m,$k,$i) {
		if($i==1)
			return ($m+$k-1)%$m;
		else
			return (run($m-1,$k,$i-1)+$k)%$m;
	}
	for ($i=1; $i <= $num ; $i++) {
		if($i!=$num)
			echo "第{$i}次出环：" .run($num,$timer,$i) , '<br/>';
		else
			echo "最后一个：" .run($num,$timer,$i) , '<br/>';
	}
}
